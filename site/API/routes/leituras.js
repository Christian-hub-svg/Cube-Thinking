...
router.get('/ultimas/:id_usuario_agora', function(req, res, next) {
	
	// quantas são as últimas leituras que quer? 7 está bom?
	const limite_linhas = 5;

	var id_usuario_agora = req.params.id_usuario_agora;

	console.log(`Recuperando os ultimos ${limite_linhas} historicos`);
	...
	if (env == 'dev') {
		...
		instrucaoSql = `select
		id_historico,
		tempo FORMAT(tempo,'HH:mm:ss') as tempo
		from historico_chris
		where fk_usuario = ${id_usuario_agora}
		...
		id_historico, 
		tempo, 
		FORMAT(tempo,'HH:mm:ss') as tempo
		from historico_chris
		where fk_usuario = ${id_usuario_agora}
